...
pub mod runners {
    ...
    pub async fn login_runner(payload: Login, app_data: AppData) -> ServiceResult<UserAuthentication> {
        let user_fut = sqlx::query_as!(UserAuthentication,
            r#"
    ...
    WHERE email = ?
        ...
        match user_fut {
            Ok(user_authentication) => {
                ...
                log::info!("password_hash = {}\npassword_usr  = {}", hash_str, user_authentication.password);
...
async fn login(
    ...
    let userAuthentication = runners::login_runner(payload.into_inner(), app_data).await?;
    // id.remember(userAuthentication.id.to_string());
    Ok(HttpResponse::Ok().json(json!({ "userAuthentication": userAuthentication })))
}